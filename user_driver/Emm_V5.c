...
void Emm_V5_Trig_Encoder_Cal(uint8_t addr)
{
  ...
  // 发送命令
	(void)Serial1_SendArrayTry((const uint8_t *)cmd, (uint16_t) 4);
}
...
void Emm_V5_Multi_Motor_Cmd(uint8_t addr)
{
  uint16_t i = 0, j = 0, len = 0; __IO static uint8_t cmd[MMCL_LEN] = {0};
	...
	if(MMCL_count > 0)
	{
		...
		// 发送命令
		(void)Serial1_SendArrayTry((const uint8_t *)cmd, (uint16_t) j); MMCL_count = 0;
	}
	else
	{
		MMCL_count = 0;
...
void Emm_V5_En_Control(uint8_t addr, bool state, bool snF)
{
  ...
  // 发送命令
  (void)Serial1_SendArrayTry((const uint8_t *)cmd, (uint16_t) 6);
}
...
void Emm_V5_Vel_Control(uint8_t addr, uint8_t dir, uint16_t vel, uint8_t acc, bool snF)
{
  ...
  // 发送命令
  (void)Serial1_SendArrayTry((const uint8_t *)cmd, (uint16_t) 8);
}
...
void Emm_V5_Pos_Control(uint8_t addr, uint8_t dir, uint16_t vel, uint8_t acc, uint32_t clk, bool raF, bool snF)
{
  ...
  // 发送命令
  (void)Serial1_SendArrayTry((const uint8_t *)cmd, (uint16_t) 13);
}

/**
  * @brief    角度换算为脉冲数
  * @param    angle_deg      ：角度幅值(单位: deg)，若为负值将按绝对值处理
  * @note     每圈脉冲数由 EMM_V5_STEP_ANGLE_X10_DEG 与 EMM_V5_MICROSTEP 自动计算
  * @retval   换算后的脉冲数(四舍五入到最近整数)
  */
uint32_t Emm_V5_Angle_To_Pulses(float angle_deg)
{
  float pulses_f;
  ...
  pulses_f = (angle_deg * (float)EMM_V5_PULSES_PER_REV) / 360.0f;
  if (pulses_f >= 4294967295.0f) {
    return 0xFFFFFFFFU;
  }
...
/**
  ...
  * @param    angle_deg      ：目标角度幅值(单位: deg)
  * @param    raF            ：相位/绝对标志，false为相对运动，true为绝对值运动
  * @param    snF            ：多机同步标志，false为不启用，true为启用
  * @retval   无
  */
void Emm_V5_Pos_Control_Angle(uint8_t addr, uint8_t dir, uint16_t vel, uint8_t acc, float angle_deg, bool raF, bool snF)
{
  uint32_t clk = Emm_V5_Angle_To_Pulses(angle_deg);
...
void Emm_V5_Stop_Now(uint8_t addr, bool snF)
{
  ...
  // 发送命令
  (void)Serial1_SendArrayTry((const uint8_t *)cmd, (uint16_t) 5);
}
...
void Emm_V5_Origin_Read_Params(uint8_t addr)
{
  ...
  // 发送命令
  (void)Serial1_SendArrayTry((const uint8_t *)cmd, (uint16_t) 3);
}
...
void Emm_V5_Origin_Modify_Params(uint8_t addr, bool svF, uint8_t o_mode, uint8_t o_dir, uint16_t o_vel, uint32_t o_tm, uint16_t sl_vel, uint16_t sl_ma, uint16_t sl_ms, bool potF)
{
  ...
  // 发送命令
  (void)Serial1_SendArrayTry((const uint8_t *)cmd, (uint16_t) 20);
}
...
void Emm_V5_Auto_Return_Sys_Params_Timed(uint8_t addr, SysParams_t s, uint16_t time_ms)
{
  ...
  // 发送命令
  (void)Serial1_SendArrayTry((const uint8_t *)cmd, (uint16_t) i);
}
...
void Emm_V5_Modify_OM_mA(uint8_t addr, bool svF, uint16_t om_ma)
{
  ...
  // 发送命令
  (void)Serial1_SendArrayTry((const uint8_t *)cmd, (uint16_t) 7);
}
...
void Emm_V5_Modify_PID_Params(uint8_t addr, bool svF, uint32_t kp, uint32_t ki, uint32_t kd)
{
  ...
  // 发送命令
  (void)Serial1_SendArrayTry((const uint8_t *)cmd, (uint16_t) 17);
}
...
void Emm_V5_Modify_DMX512_Params(uint8_t addr, bool svF, uint16_t tch, uint8_t nch, uint8_t mode, uint16_t vel, uint16_t acc, uint16_t vel_step, uint32_t pos_step)
{
  ...
  // 发送命令
  (void)Serial1_SendArrayTry((const uint8_t *)cmd, (uint16_t) 19);
}
...
void Emm_V5_Modify_Otocp(uint8_t addr, bool svF, uint16_t otp, uint16_t ocp, uint16_t time_ms)
{
  ...
  // 发送命令
  (void)Serial1_SendArrayTry((const uint8_t *)cmd, (uint16_t) 11);
}
...
void Emm_V5_Modify_Heart_Protect(uint8_t addr, bool svF, uint32_t hp)
{
  ...
  // 发送命令
  (void)Serial1_SendArrayTry((const uint8_t *)cmd, (uint16_t) 9);
}
