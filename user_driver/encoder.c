...
static volatile int32_t encoder_total_count[2] = {0, 0};
...
void Encoder_Record_Pulse(uint8_t motor_index)
{
    if (motor_index < 2U) {
        ...
        encoder_total_count[motor_index]++;
    }
}
...
/** Return the accumulated A-phase rising-edge count since Encoder_Init(). */
int32_t Encoder_Get_Total_Count(uint8_t motor_id)
{
    return motor_id < 2U ? encoder_total_count[motor_id] : 0;
}
