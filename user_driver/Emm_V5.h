...
#define         EMM_V5_STEP_ANGLE_X10_DEG     (18U)   // 1.8deg -> 18, 0.9deg -> 9
#define         EMM_V5_MICROSTEP              (16U)   // 电机细分数
#define         EMM_V5_PULSES_PER_REV         ((uint32_t)(((3600U / EMM_V5_STEP_ANGLE_X10_DEG) * EMM_V5_MICROSTEP)))
...
// 角度换算为脉冲数（angle_deg为角度幅值，方向由dir参数决定）
uint32_t Emm_V5_Angle_To_Pulses(float angle_deg);
// 位置模式角度控制（内部自动执行角度->脉冲换算）
