...
#define MOTOR_PID_KP                 (40) /* 比例 0.40；增大后跟速更快，过大会振荡。 */
#define MOTOR_PID_KI                 (20) /* 积分 0.20；消除速度静差，过大会过冲。 */
#define MOTOR_PID_KD                 (0)  /* 微分 0.00；编码器脉冲较稀疏，默认关闭。 */

/* PID may add this much PWM above the requested speed, but cannot reach 100% from a 40% target. */
#define MOTOR_PID_DUTY_HEADROOM_PERCENT (15) /* PID 可额外增加的 PWM 百分比。 */
